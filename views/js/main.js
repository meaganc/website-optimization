...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    ...
    // All pizzas displayed are the same size; only select the first pizza for base width
...
// All pizzas are being appended to the main div, no need to reinitialize
...
function logAverageFrame(times) {   // times is the array of User Timing measurements from updatePositions()
  ...
  for (var i = numberOfEntries - 1; i > numberOfEntries - 11; i--) {
    // Make sure the time is set before determining duration
    if (times[i]) {
...
function updatePositions() {
  frame++;
  // Update for the first time to properly place the pizzas, and then only every other frame
  if (frame === 1 || frame % 2 === 0) {
    ...
    for (var i = 0; i < items.length; i++) {
      phase = Math.sin((scrollVal) + (i % 5));
      //Switch from left to transform
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  //Reduce the number of created pizzas
  var elem;
  for (var i = 0; i < 50; i++) {
